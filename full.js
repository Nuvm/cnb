...
API.chatLog("「UB」is now running!", true);
var startMsg = "「UB」v1.1 is now running! Type !cmd for a list of commands.";
...
function full(data){
  ...
  if (commandWait === true){
    switch(data.message){
      ...
      case'!disable extra':
        if(data.uid === 5010460){
            API.sendChat("/me 「UB」has been disabled by " + userName3 + ".");
